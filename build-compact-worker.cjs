...
// 1. Bundle App.jsx into an IIFE
console.log("Bundling App.jsx as IIFE...");
const appIife = execSync(
  "npx esbuild src/App.jsx --bundle --minify --format=iife --global-name=OutreachApp --external:react --external:react-dom",
  { cwd: path.join(__dirname, "client") }
).toString().trim();

// 2. Read and minify style.css
const cssRaw = fs.readFileSync(path.join(__dirname, "client", "src", "style.css"), "utf8");
const cssMin = cssRaw.replace(/\s+/g, " ").replace(/\/\*.*?\*\//g, "").trim();

// 3. Build single self-contained index.html
...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ${appIife}
      const RootApp = (typeof OutreachApp !== 'undefined' && OutreachApp.default) ? OutreachApp.default : OutreachApp;
...
// 4. Build Cloudflare Worker code (using service worker format addEventListener('fetch', ...))
// Using addEventListener syntax ensures it works 100% reliably in all Cloudflare Worker API upload modes!
const workerCode = `// InfoSkills Outreach - Cloudflare Worker
...
export default {
  async fetch(request, env) {
    ...
    // Default to serving the single-page application
...
fs.writeFileSync(path.join(__dirname, "compact-worker.js"), workerCode, "utf8");
console.log("SUCCESS: Created compact-worker.js! Total size:", Buffer.byteLength(workerCode), "bytes");
